...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton("Cʜᴀɴɴᴇʟ 🔈", url="https://t.me/Linux_Bots")
            ],[      
            InlineKeyboardButton("Hᴇʟᴩ 🕸️", callback_data="help"),
            InlineKeyboardButton("Aʙᴏᴜᴛ ✨", callback_data="about")
        ]]
        m = await message.reply_sticker("CAACAgUAAxkBAAENw7Rnqw3eceR7a0d4VUdnGLs05NbHpgAC6BAAAgSZYVWPjWbf1ZGIJjYE") 
        await asyncio.sleep(2)
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        ...
        m = await message.reply_sticker("CAACAgQAAxkBAAJt6WWj9vGs5G2CoaycrCo5giuMerOgAAJUDAAC7JjgU96PDZ4e0VqjNAQ")
        await asyncio.sleep(2)
        await message.reply_photo(photo=random.choice(PICS), caption=START_MESSAGE.format(user=message.from_user.mention, bot=client.mention), reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
        return await m.delete()
